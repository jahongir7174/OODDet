...
def rotate(image, magnitude):
    magnitude = (magnitude / max_value) * 5.00
...
def shear_x(image, magnitude):
    magnitude = (magnitude / max_value) * 0.03
...
def translate_x(image, magnitude):
    magnitude = (magnitude / max_value) * 0.05
...
class MixAugment:
    def __init__(self, mean=3, sigma=0.5, n=3):
        self.n = n
        self.mean = mean
        self.sigma = sigma
...
class ROBINDataset(CocoDataset):
    CLASSES = ('aeroplane',
               'bicycle',
               'boat',
               'bus',
